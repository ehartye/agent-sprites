...
const OUTFITS=['casual','field','service','retro'],DIRECTIONS=['down','right','up','left'],HAIR=['short','bun','bob','waves','puffs','tousled'];
...
export function generateCharacterRecipe(config){
  ...
  const people=config.people.map(p=>{
    object(p,'person',['id','body','hair','skin','colors']);
  ...
  for(const person of people)for(const outfit of outfits)for(const direction of directions){
    ...
    for(let index=0;index<(mode==='idle'?1:8);index++){
      ...
      const add=(type,part,color,fields,points)=>{
        part=mirror?part.replace(/^(left|right)_/,s=>s==='left_'?'right_':'left_'):part;
      ...
      frames.push({cell,alias,person:person.id,body:person.body,outfit,expression,...pose,sealed:outfit!=='casual',bounds,checks});
    }
